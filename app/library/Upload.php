...
/**
 * Source http://www.jb51.net/article/57510.htm
 * Class Upload
 * Package Front
 */
class Upload
{
    ...
    private $__basepath = UPLOAD_PATH;//上传文件保存的路径
    private $allowtype = ['jpg','gif','png'];//设置限制上传文件的类型
    ...
    /**
     * 用于设置成员属性（$allowtype,$maxsize, $israndname）
     * 可以通过连贯操作一次设置多个属性值
    ...
    {
        $key = strtolower($key);
        if(in_array($key,['maxsize','israndname','allowtype']))
        {
    ...
    private function proRandName() {
        $fileName = md5(microtime(true).mt_rand(0,9999));
